public class Ordinatore 
{
	...
	public static int scambia(Intervento[]array,int pos1,int pos2)
	{
		Intervento s;
		if(pos1<0 || pos2<0 || pos1>=array.length || pos2>=array.length)
			return -1;
	...
	private static Intervento[] copiaArray(Intervento[]array)
	{
		Intervento[]arrayCopia=new Intervento[array.length];
		for (int i = 0; i < arrayCopia.length; i++) 
			arrayCopia[i] = array[i];
		return arrayCopia;
	}
	...
	public static Intervento[] selectionSortCrescente(Intervento[] array)
	{
		Intervento[] arrayCopia=copiaArray(array);
		for (int i = 0; i < arrayCopia.length-1; i++) 
		{
			for (int j = i+1; j < arrayCopia.length; j++) 
			{
				if (arrayCopia[i].getCodiceidentificativointervento().compareTo(arrayCopia[j].getCodiceidentificativointervento())>0)
					scambia(arrayCopia, i, j);
			}
		}
		return arrayCopia;
